...
public class ReserverenServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		Voorstelling voorstelling = voorstellingenDAO.read(Integer
		...
		if (session.getAttribute("mandje") != null) {
			...
			if (mandje.containsKey(voorstellingid)) {
				request.setAttribute("aantalPlaatsen",
						mandje.get(voorstellingid));
	...
	protected void doPost(HttpServletRequest request,
		...
		if (voorstelling != null) {
			int aantalPlaatsen = Integer.parseInt(request
					.getParameter("aantalPlaatsen"));
			if (aantalPlaatsen >= 1
					&& aantalPlaatsen <= voorstelling.getVrijeplaatsen()) {
				@SuppressWarnings("unchecked")
				Map<Integer, Integer> mandje = (Map<Integer, Integer>) session
						.getAttribute(MANDJE);
				if (mandje == null) {
